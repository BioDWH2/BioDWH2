 ...
 * <p>
 * Chunks are page aligned (each page is usually 4096 bytes). There are at most 67 million (2^26) chunks, each chunk is
 * at most 2 GB large.
 */
public class Chunk {
    ...
    /**
     * The length of the chunk footer. The longest footer is: chunk:ffffffff,block:ffffffffffffffff,
    ...
    /**
     * When this chunk was no longer needed, in milliseconds after the store was created. After this, the chunk is kept
     * alive a bit longer (in case it is referenced in older versions).
     */
    public long unused;

    /**
     * Version of the store at which chunk become unused and therefore can be considered "dead" and collected after this
    ...
    boolean isRewritable() {
        return isSaved() && isLive() && pageCountLive < pageCount    // not fully occupied
               && isEvacuatable();
    }
    ...
    ByteBuffer readBufferForPage(FileStore fileStore, long pos, int expectedMapId) {
        ...
        while (true) {
            ...
            try {
                ...
                if (filePos < 0) {
                    throw DataUtils.newIllegalStateException(DataUtils.ERROR_FILE_CORRUPT,
                                                             "Negative position {0}; p={1}, c={2}", filePos, pos,
                                                             toString());
                }
                ...
                if (pageLength > remaining || pageLength < 4) {
                    throw DataUtils.newIllegalStateException(DataUtils.ERROR_FILE_CORRUPT,
                                                             ...
                                                             id, remaining, pageLength);
                }
                buff.limit(start + pageLength);
                short check = buff.getShort();
                int checkTest = DataUtils.getCheckValue(id) ^ DataUtils.getCheckValue(offset) ^ DataUtils.getCheckValue(
                        pageLength);
     ...
     * @param now        is a moment in time (since creation of the store), when removal is recorded, and retention
     *                   period starts
