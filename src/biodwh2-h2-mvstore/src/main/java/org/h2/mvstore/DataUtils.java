...
public final class DataUtils {
    ...
    /**
     * The prefix for chunks ("chunk."). This, plus the chunk id (hex encoded) is the key, and the serialized chunk
     * metadata is the value.
     */
    public static final String META_CHUNK = "chunk.";

    /**
     * The prefix for names ("name."). This, plus the name of the map, is the key, and the map id (hey encoded) is the
     * value.
     */
    public static final String META_NAME = "name.";

    /**
     * The prefix for maps ("map."). This, plus the map id (hex encoded) is the key, and the serialized in the map
     * metadata is the value.
     */
    public static final String META_MAP = "map.";

    /**
     * The prefix for root positions of maps ("root."). This, plus the map id (hex encoded) is the key, and the position
     * (hex encoded) is the value.
     */
    ...
    public static void readFully(FileChannel file, long pos, ByteBuffer dst) {
        try {
            ...
            throw newIllegalStateException(ERROR_READING_FAILED, "Reading from file {0} failed at {1} (length {2}), " +
                                                                 "read {3}, remaining {4}", file, pos, size,
                                           dst.position(), dst.remaining(), e);
        }
    }
    ...
    /**
     * Get the position of this page. The following information is encoded in the position: the chunk id, the offset,
     * the maximum length, and the type (node or leaf).
     *
    ...
    /**
     * Calculate a check value for the given integer. A check value is mean to verify the data is consistent with a high
     * probability, but not meant to protect against media failure or deliberate changes.
     *
     * @param x the value
     ...
     * @param map  the map
     */
    public static void appendMap(StringBuilder buff, HashMap<String, ?> map) {
    ...
    /**
     * Append a key-value pair to the string builder. Keys may not contain a colon. Values that contain a comma or a
     * double quote are enclosed in double quotes, with special characters escaped using a backslash.
     *
     * @param buff  the target buffer
     * @param key   the key
     ...
     * @return the map without mapping for {@code "fletcher"}, or {@code null} if checksum is wrong or parameter do not
     * represent a properly formatted map serialization
     */
    ...
    public static String getMapName(String s) {
        ...
        for (int i = 0, size = s.length(); i < size; ) {
            ...
            if (i++ - startKey == keyLength && s.regionMatches(startKey, "name", 0, keyLength)) {
                StringBuilder buff = new StringBuilder();
