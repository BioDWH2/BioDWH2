 ...
 * <p>
 * File format: page length (including length): int check value: short map id: varInt number of keys: varInt type: byte
 * (0: leaf, 1: node; +2: compressed) compressed: bytes saved (varInt) keys leaf: values (one for each key) node:
...
public abstract class Page implements Cloneable {
    ...
    /**
     * Position of this page's saved image within a Chunk or 0 if this page has not been saved yet or 1 if this page has
     * not been saved yet, but already removed This "removed" flag is to keep track of pages that concurrently changed
     * while they are being stored, in which case the live bookkeeping needs to be aware of such cases. Field need to be
     * volatile to avoid races between saving thread setting it and other thread reading it to access the page. On top
     * of this update atomicity is required so removal mark and saved position can be set concurrently
     */
    ...
    // Java 6, 64 bit: 24
    // Java 6, 32 bit: 12
    static final int MEMORY_OBJECT = 24;

    /**
     * The memory needed by an array.
     */
    static final int MEMORY_ARRAY = 24;
    ...
    // Java 6, 64 bit: 8
    // Java 6, 32 bit: 4
    static final int MEMORY_POINTER = 8;
    /**
     ...
     * @return the page
     */
    public static Page createNode(MVMap<?, ?> map, Object[] keys, PageReference[] children, long totalCount,
                                  int memory) {
        assert keys != null;
    ...
    /**
     * Search the key in this page using a binary search. Instead of always starting the search in the middle, the last
     * found index is cached.
     * <p>
     * If the key was found, the returned value is the index in the key array. If not found, the returned value is
     * negative, where -1 means the provided key is smaller than any keys in this page. See also Arrays.binarySearch.
     *
     * @param key the key
    ...
    private void read(ByteBuffer buff, int chunkId) {
        // size of int + short + variant, since we've read page length, check and
        // mapId already
        ...
        if (compressed) {
            CompressLZF compressor = map.getStore().getCompressorFast();
    ...
    /**
     * Mark this page as removed "in memory". That means that only adjustment of "unsaved memory" amount is required. On
     * the other hand, if page was persisted, it's removal should be reflected in occupancy of the containing chunk.
     *
     * @return true if it was marked by this call or has been marked already, false if page has been saved already.
     */
    ...
    protected final int write(Chunk chunk, WriteBuffer buff) {
        ...
        if (expLen > 16) {
            ...
            if (compLen + plus < expLen) {
                buff.position(typePos).put((byte) (type + DataUtils.PAGE_COMPRESSED));
                buff.position(compressStart).putVarInt(expLen - compLen).put(comp, 0, compLen);
            }
        }
        int pageLength = buff.position() - start;
        int chunkId = chunk.id;
        int check = DataUtils.getCheckValue(chunkId) ^ DataUtils.getCheckValue(start) ^ DataUtils.getCheckValue(
                pageLength);
        ...
        chunk.accountForWrittenPage(pageLengthEncoded);
        if (isDeleted) {
            store.accountForRemovedPage(pagePos, chunk.version + 1);
        }
        return typePos + 1;
    }
     ...
     * @return amount (negative), by which "unsaved memory" should be adjusted, if page is unsaved one, and 0 for page
     * that was already saved, or in case of non-persistent map
     */
    public final int removePage(long version) {
        if (isPersistent() && getTotalCount() > 0) {
            ...
            if (!markAsRemoved()) { // only if it has been saved already
                long pagePos = pos;
                store.accountForRemovedPage(pagePos, version);
            } else {
                return -memory;
            }
        }
    ...
    public static final class PageReference {
        ...
        /**
         * Clear if necessary, reference to the actual child Page object, so it can be garbage collected if not actively
         * used elsewhere. Reference is cleared only if corresponding page was already saved on a disk.
         */
        ...
        public String toString() {
            return "Cnt:" + count + ", pos:" + (pos == 0 ? "0" : DataUtils.getPageChunkId(pos) + "-" +
                                                                 DataUtils.getPageOffset(pos) + ":" +
                                                                 DataUtils.getPageMaxLength(pos)) +
                ...
                for (int i = 0, size = map.getChildPageCount(this); i < size; i++) {
                    ...
                    if (page != null) {
                        ...
                        if (DataUtils.isLeafPosition(pagePos)) {
                            map.store.accountForRemovedPage(pagePos, version);
                        } else {
