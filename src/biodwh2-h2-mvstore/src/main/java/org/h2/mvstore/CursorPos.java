...
/**
 * A position in a cursor. Instance represents a node in the linked list, which traces path from a specific (target) key
 * within a leaf node all the way up to te root (bottom up path).
 */
public class CursorPos {
    ...
    /**
     * Index of the key (within page above) used to go down to a lower level in case of intermediate nodes, or index of
     * the target key for leaf a node. In a later case, it could be negative, if the key is not present.
     */
    public int index;

    /**
     * Next node in the linked list, representing the position within parent level, or null, if we are at the root level
     * already.
     */
    ...
    /**
     * Searches for a given key and creates a breadcrumb trail through a B-tree rooted at a given Page. Resulting path
     * starts at "insertion point" for a given key and goes back to the root.
     *
