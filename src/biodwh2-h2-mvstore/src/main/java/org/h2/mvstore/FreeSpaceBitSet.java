...
public class FreeSpaceBitSet {
    ...
    /**
     * Left-shifting register, which holds outcomes of recent allocations. Only allocations done in "reuseSpace" mode
     * are recorded here. For example, rightmost bit set to 1 means that last allocation failed to find a hole big
     * enough, and next bit set to 0 means that previous allocation request have found one.
     */
    ...
    public void free(long pos, int length) {
        int start = getBlock(pos);
        int blocks = getBlockCount(length);
        assert set.nextClearBit(start) >= start + blocks : "Double free: " + Integer.toHexString(start) + "/" +
    ...
    private int getBlockCount(int length) {
        return Utils.roundUpInt(length, blockSize) / blockSize;
    }

    /**
     * Get the fill rate of the space in percent. The value 0 means the space is completely free, and 100 means it is
     * completely full.
     *
     ...
     *
     * @param vacatedBlocks number of blocks vacated  as a result of live data evacuation less number of blocks in
     *                      prospective chunk with evacuated live data
    ...
    int getMovePriority(int block) {
        // The most desirable chunks to move are the ones sitting within a relatively short span of occupied blocks
        // which is surrounded from both sides by relatively long free spans
    ...
    public String toString() {
        ...
        if (DETAILED_INFO) {
            ...
            buff.append('\n').append(" on ").append(onCount).append(" off ").append(offCount).append(' ').append(
                    100 * onCount / (onCount + offCount)).append("% used ");
        }
        buff.append('[');
        for (int i = 0; ; ) {
            if (i > 0) {
