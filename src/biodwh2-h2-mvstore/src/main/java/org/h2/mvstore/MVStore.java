...
public class MVStore implements AutoCloseable {
    ...
    /**
     * The block size (physical sector size) of the disk. The store header is written twice, one copy in each block, to
     * ensure it survives a crash.
     */
    ...
    /**
     * Store is about to close now, but is still operational. Outstanding store operation by background writer or other
     * thread may be in progress. New updates must not be initiated, unless they are part of a closing procedure
     * itself.
     */
    ...
    private static final int VERSIONS_TO_KEEP = 5;

    public static final int KEYS_PER_PAGE = 48;

    /**
     * Lock which governs access to major store operations: store(), close(), ... It should used in a non-reentrant
     * fashion. It serves as a replacement for synchronized(this), except it allows for non-blocking lock attempts.
     */
    ...
    /**
     * The page cache. The default size is 16 MB, and the average size is 2 KB. It is split in 16 segments. The stack
     * move distance is 2% of the expected number of entries.
     */
    ...
    private final CompressLZF compressorFast = new CompressLZF();
    ...
    /**
     * Ordered collection of all version usage counters for all versions starting from oldestVersionToKeep and up to
     * current.
     */
    ...
    /**
     * The estimated memory used by unsaved pages. This number is not accurate, also because it may be changed
     * concurrently, and because temporary pages are counted.
     */
    ...
    /**
     * How long to retain old, persisted chunks, in milliseconds. For larger or equal to zero, a chunk is never directly
     * overwritten if unused, but instead, the unused field is set. If smaller zero, chunks are directly overwritten if
     * unused.
     */
    ...
    private final int autoCompactFillRate = 90;
     ...
     */
    public MVStore(final String fileName, final boolean readOnly) {
        fileStore = new FileStore();
        cache = new CacheLongKeyLIRS<>();
        meta = new MVMap<>(this);
        ...
        try {
            fileStore.open(fileName, readOnly);
            if (fileStore.size() == 0) {
    ...
    /**
     * Open a map with the default settings. The map is automatically create if it does not yet exist. If a map with
     * this name is already open, this map is returned.
     *
    ...
    public <K, V> MVMap<K, V> openMap(String name) {
        ...
        if (id >= 0) {
            ...
            markMetaChanged();
            @SuppressWarnings("unchecked") MVMap<K, V> existingMap = (MVMap<K, V>) maps.putIfAbsent(id, map);
            if (existingMap != null) {
                map = existingMap;
            }
        }
    ...
    private <K, V> MVMap<K, V> openMap(int id, MVMap.Builder<K, V> builder) {
        storeLock.lock();
        try {
            MVMap<K, V> map = getMap(id);
            if (map == null) {
    ...
    public <K, V> MVMap<K, V> getMap(int id) {
        checkOpen();
        @SuppressWarnings("unchecked") MVMap<K, V> map = (MVMap<K, V>) maps.get(id);
        return map;
    }
    ...
    private void readStoreHeader() {
        ...
        if (format > FORMAT_WRITE && !fileStore.isReadOnly()) {
            throw DataUtils.newIllegalStateException(DataUtils.ERROR_UNSUPPORTED_FORMAT,
                                                     ...
                                                     "and the file was not opened in read-only mode", format,
                                                     FORMAT_WRITE);
        }
        ...
        assumeCleanShutdown = assumeCleanShutdown && newest != null && DataUtils.readHexInt(storeHeader, HDR_CLEAN,
                                                                                            0) != 0;
    ...
    private boolean findLastChunkWithCompleteValidChunkSet(Chunk[] lastChunkCandidates,
                                                           ...
                                                           Map<Integer, Chunk> validChunksById, boolean afterFullScan) {
     ...
     *
     * @param block to start search from (found chunk footer should be no further than block-1)
     ...
     * @return the chunk, or null if the header or footer don't match or are not consistent
     */
    ...
    /**
     * Close the file and the store. Unsaved changes are written to disk first, and compaction (up to a specified number
     * of milliseconds) is attempted.
     *
     * @param allowedCompactionTime the allowed time for compaction (in milliseconds)
     */
    ...
    /**
     * Close the file and the store, without writing anything. This will try to stop the background thread (without
     * waiting for it). This method ignores all errors.
     */
    ...
    private Chunk getChunk(long pos) {
        ...
        if (c == null) {
            ...
            if (s == null) {
                throw DataUtils.newIllegalStateException(DataUtils.ERROR_CHUNK_NOT_FOUND, "Chunk {0} not found",
                                                         chunkId);
            }
    ...
    /**
     * Unlike regular commit this method returns immediately if there is commit in progress on another thread, otherwise
     * it acts as regular commit.
     * <p>
     ...
     * Commit the changes.
     * <p>
     * This method does nothing if there are no unsaved changes, otherwise it increments the current version and stores
     * the data (for file based stores).
     * <p>
     * It is not necessary to call this method when auto-commit is enabled (the default setting), as in this case it is
     * automatically called from time to time or when enough changes have accumulated. However, it may still be called
     * to flush all changes to disk.
     * <p>
    ...
    private void storeNow(long reservedLow, long reservedHigh) {
        ...
        while (true) {
            ...
            if (!old.isSaved()) {
                IllegalStateException e = DataUtils.newIllegalStateException(DataUtils.ERROR_INTERNAL,
                                                                             "Last block {0} not stored, possibly due to out-of-memory",
                                                                             old);
                panic(e);
            }
        }
        ...
        // will have its id reflected in mapid field of currently written chunk
        c.mapId = lastMapId.get();

        int chunkLength = buff.position();

        // add the store header and round to the next block
        int length = Utils.roundUpInt(chunkLength + Chunk.FOOTER_LENGTH, BLOCK_SIZE);
        buff.limit(length);
    ...
    /**
     * Get a buffer for writing. This caller must synchronize on the store before calling the method and until after
     * using the buffer.
     *
     * @return the buffer
     */
    ...
    /**
     * Release a buffer for writing. This caller must synchronize on the store before calling the method and until after
     * using the buffer.
     *
    ...
    /**
     * Apply the freed space to the chunk metadata. The metadata is updated, but completely free chunks are not removed
     * from the set of chunks, and the disk space is not yet marked as free. They are queued instead and wait until
     * their usage is over.
     */
    ...
    /**
     * Get the index of the first block after last occupied one. It marks the beginning of the last (infinite) free
     * space.
     *
     * @return block index
     */
    ...
    /**
     * Compact the store by moving all chunks next to each other, if there is free space between chunks. This might
     * temporarily increase the file size. Chunks are overwritten irrespective of the current retention time. Before
    ...
    private void compactMoveChunks(Iterable<Chunk> move) {
        ...
        if (move != null) {
            ...
            for (Chunk c : move) {
                if (c.block >= originalBlockCount && moveChunk(c, originalBlockCount, postEvacuationBlockCount)) {
    ...
    private boolean moveChunkInside(Chunk chunkToMove, long boundary) {
        boolean res = chunkToMove.block >= boundary && fileStore.predictAllocation(chunkToMove.len, boundary, -1) <
                                                       boundary && moveChunk(chunkToMove, boundary, -1);
        assert !res || chunkToMove.block + chunkToMove.len <= boundary;
        return res;
    }

    /**
     * Move specified chunk into free area of the file. "Reserved" area specifies file interval to be avoided, when
     * un-allocated space will be chosen for a new chunk's location.
     *
     * @param chunk            to move
    ...
    /**
     * Compact store file, that is, compact blocks that have a low fill rate, and move chunks next to each other. This
     * will typically shrink the file. Changes are flushed to the file, and old chunks are overwritten.
     *
    ...
    /**
     * Try to increase the fill rate by re-writing partially full chunks. Chunks with a low number of live items are
     * re-written.
     * <p>
     ...
     * <p>
     * Only data of open maps can be moved. For maps that are not open, the old chunk is still referenced. Therefore, it
     * is recommended to open all maps before calling this method.
     *
            ...
            if (getChunksFillRate() < 95) {
                ...
                try {
                    if (storeLock.tryLock(10, TimeUnit.MILLISECONDS)) {
                        try {
                            return rewriteChunks(16 * 1024 * 1024);
                        } finally {
                            storeLock.unlock();
                        }
                    }
    ...
    /**
     * Get the current fill rate (percentage of used space in the file). Unlike the fill rate of the store, here we only
     * account for chunk data; the fill rate here is how much of the chunk data is live (still referenced). Young chunks
     * are considered live.
     *
    ...
    private Iterable<Chunk> findOldChunks(int writeLimit) {
        ...
        PriorityQueue<Chunk> queue = new PriorityQueue<>(this.chunks.size() / 4 + 1, (o1, o2) -> {
     ...
     */
    void accountForRemovedPage(long pos, long version) {
        assert DataUtils.isPageSaved(pos);
        RemovedPageInfo rpi = new RemovedPageInfo(pos, false, version);
        removedPages.add(rpi);
    }

    CompressLZF getCompressorFast() {
    ...
    /**
     * How long to retain old, persisted chunks, in milliseconds. Chunks that are older may be overwritten once they
     * contain no live data.
     * <p>
     * The default value is 45000 (45 seconds) when using the default file store. It is assumed that a file system and
     * hard disk will flush all write buffers within this time. Using a lower value might be dangerous, unless the file
     * system and hard disk flush the buffers earlier. To manually flush the buffers, use
     * <code>MVStore.getFile().force(true)</code>, however please note that
     * according to various tests this does not always work as expected depending on the operating system and hardware.
     * <p>
     * The retention time needs to be long enough to allow reading old chunks while traversing over the entries of a
     * map.
     * <p>
     * This setting is not persisted.
     *
     * @param ms how many milliseconds to retain old chunks (0 to overwrite them as early as possible)
     */
    ...
    /**
     * Get the oldest version to retain. We keep at least number of previous versions specified by "versionsToKeep"
     * configuration parameter (default 5). Previously it was used only in case of non-persistent MVStore. Now it's
     * honored in all cases (although H2 always sets it to zero). Oldest version determination also takes into account
     * calls (de)registerVersionUsage(), and will not release the version, while version is still in use.
     *
     * @return the version
     */
    long getOldestVersionToKeep() {
        long v = oldestVersionToKeep.get();
        v = Math.max(v - VERSIONS_TO_KEEP, INITIAL_VERSION);
        if (fileStore != null) {
    ...
    private void setOldestVersionToKeep(long oldestVersionToKeep) {
        ...
        do {
            ...
            success = oldestVersionToKeep <= current || this.oldestVersionToKeep.compareAndSet(current,
                                                                                               oldestVersionToKeep);
        } while (!success);
    }

    /**
     * Adjust amount of "unsaved memory" meaning amount of RAM occupied by pages not saved yet to the file. This is the
     * amount which triggers auto-commit.
     *
    ...
    /**
     * Performs final stage of map removal - delete root location info from the meta table. Map is supposedly closed and
     * anonymous and has no outstanding usage by now.
     *
            ...
            try {
                ...
                for (int cnt = 0; cnt < 5; cnt++) {
                    ...
                    if (fillRate > autoCompactFillRate) {
                        ...
                        if (projectedFillRate > autoCompactFillRate || projectedFillRate <= lastProjectedFillRate) {
                            break;
                        }
                    }
                    ...
                    try {
                        int writeLimit = autoCommitMemory * autoCompactFillRate / Math.max(projectedFillRate, 1);
     ...
     * <p>
     * To disable auto-commit, set the value to 0. In this case, changes are only committed when explicitly calling
     * commit.
     * <p>
    ...
    /**
     * Register opened operation (transaction). This would increment usage counter for the current version. This version
     * (and all after it) should not be dropped until all transactions involved are closed and usage counter goes to
     * zero.
     *
    ...
    /**
     * De-register (close) completed operation (transaction). This will decrement usage counter for the corresponding
     * version. If counter reaches zero, that version (and all unused after it) can be dropped immediately.
     *
    ...
    private void dropUnusedVersions() {
        assert storeLock.isHeldByCurrentThread();
        TxCounter txCounter;
        while ((txCounter = versions.peek()) != null && txCounter.get() < 0) {
            versions.poll();
        }
    ...
    private int dropUnusedChunks() {
        ...
        if (!deadChunks.isEmpty()) {
            ...
            long time = getTimeSinceCreation();
            Chunk chunk;
            while ((chunk = deadChunks.poll()) != null && (isSeasonedChunk(chunk, time) && canOverwriteChunk(chunk,
                                                                                                             oldestVersionToKeep) ||
    ...
    /**
     * Class TxCounter is a simple data structure to hold version of the store along with the counter of open
     * transactions, which are still operating on this version.
     */
    public static final class TxCounter {
        ...
        private static final AtomicIntegerFieldUpdater<TxCounter> counterUpdater = AtomicIntegerFieldUpdater.newUpdater(
                TxCounter.class, "counter");
    ...
    private static class RemovedPageInfo implements Comparable<RemovedPageInfo> {
        ...
        /**
         * Transforms saved page position into removed page info, by eliminating page offset and replacing "page type"
         * bit with "pinned page" flag. 0    "pinned" flag 1-5  encoded page length 6-31 chunk id
         *
        ...
        @Override
        public String toString() {
            return "RemovedPageInfo{" + "version=" + version + ", chunk=" + getPageChunkId() + ", len=" +
                   getPageLength() + (isPinned() ? ", pinned" : "") + '}';
        }
    }
}
