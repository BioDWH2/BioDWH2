...
/**
 * Class RootReference is an immutable structure to represent state of the MVMap as a whole (not related to a particular
 * B-Tree node). Single structure would allow for non-blocking atomic state change. The most important part of it is a
...
public final class RootReference {
    ...
    /**
     * Reference to the previous root in the chain. That is the last root of the previous version, which had any data
     * changes. Versions without any data changes are dropped from the chain, as it built.
     */
     ...
     */
    RootReference updatePageAndLockedStatus(Page page, int appendCounter) {
        assert isLockedByCurrentThread() : this;
        RootReference updatedRootReference = new RootReference(this, page, false, appendCounter);
        if (root.map.compareAndSetRoot(this, updatedRootReference)) {
            return updatedRootReference;
        }
        return null;
    ...
    long getVersion() {
        ...
        return prev == null || prev.root != root || prev.appendCounter != appendCounter ? version : prev.version;
    }
    ...
    public String toString() {
        return "RootReference(" + System.identityHashCode(root) + ", v=" + version + ", owner=" + ownerId +
               (ownerId == Thread.currentThread().getId() ? "(current)" : "") + ", holdCnt=" + holdCount + ", keys=" +
               root.getTotalCount() + ", append=" + getAppendCounter() + ")";
    }
}
