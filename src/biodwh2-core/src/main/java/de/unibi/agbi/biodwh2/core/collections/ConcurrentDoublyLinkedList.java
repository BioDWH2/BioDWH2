...
/**
 * A concurrent linked-list implementation of a {@link Deque} (double-ended queue). Concurrent insertion, removal, and
 * access operations execute safely across multiple threads. Iterators are <i>weakly consistent</i>, returning elements
 * reflecting the state of the deque at some point at or since the creation of the iterator. They do <em>not</em> throw
 * {@link ConcurrentModificationException}, and may proceed concurrently with other operations.
 *
 * <p>
 * This class and its iterators implement all of the <em>optional</em> methods of the {@link Collection} and {@link
 * Iterator} interfaces. Like most other concurrent collection implementations, this class does not permit the use of
 * <tt>null</tt> elements. because some null arguments and return values
 * cannot be reliably distinguished from the absence of elements. Arbitrarily, the {@link Collection#remove} method is
 * mapped to
...
public class ConcurrentDoublyLinkedList<E> extends AbstractCollection<E> implements Serializable {
    ...
    /**
     * Appends the given element to the end of this deque. (Identical in function to the <tt>add</tt> method; included
    ...
    /**
     * Removes the first element <tt>e</tt> such that <tt>o.equals(e)</tt>, if such an element exists in this deque. If
    ...
    /**
     * Removes the last element <tt>e</tt> such that <tt>o.equals(e)</tt>, if such an element exists in this deque. If
     * the deque does not contain the element, it is unchanged.
     *
     * @param o element to be removed from this deque, if present.
     * @return <tt>true</tt> if the deque contained the specified element.
    ...
    /**
     * Returns the number of elements in this deque. If this deque contains more than <tt>Integer.MAX_VALUE</tt>
     * elements, it returns
     * <tt>Integer.MAX_VALUE</tt>.
     *
     * <p>
     * Beware that, unlike in most collections, this method is <em>NOT</em> a constant-time operation. Because of the
     * asynchronous nature of these deques, determining the current number of elements requires traversing them all to
     * count them. Additionally, it is possible for the size to change during execution of this method, in which case
     * the returned result will be inaccurate. Thus, this method is typically not very useful in concurrent
     * applications.
     *
    ...
    /**
     * Appends all of the elements in the specified collection to the end of this deque, in the order that they are
     ...
     * undefined if the specified Collection is this deque, and this deque is nonempty.)
     *
    ...
    /**
     * Returns an array containing all of the elements in this deque in the correct order; the runtime type of the
     ...
     * <p>
     * <p>
     * If the deque fits in the specified array with room to spare (i.e., the array has more elements than the deque),
     * the element in the array immediately following the end of the collection is set to null. This is useful in
     * determining the length of the deque <i>only</i> if the caller knows that the deque does not contain any null
     * elements.
     *
     ...
     * @throws ArrayStoreException  if the runtime type of a is not a supertype of the runtime type of every element in
     *                              this deque.
    ...
    /**
     * Returns a weakly consistent iterator over the elements in this deque, in first-to-last order. The <tt>next</tt>
     * method returns elements reflecting the state of the deque at some point at or since the creation of the iterator.
     * The method does <em>not</em> throw {@link ConcurrentModificationException}, and may proceed concurrently with
     * other operations.
     *
