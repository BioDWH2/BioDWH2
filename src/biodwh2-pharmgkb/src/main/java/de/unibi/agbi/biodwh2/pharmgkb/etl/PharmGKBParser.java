...
public class PharmGKBParser extends Parser<PharmGKBDataSource> {
    ...
    private void parseZipFileEntry(PharmGKBDataSource dataSource, String filePath, ZipEntry zipEntry,
        ...
        if (filePath.contains("pathways") && zipEntryName.startsWith("PA")) {
            ...
            dataSource.variantDrugAnnotations = parseTSVUnivocity(stream, VariantDrugAnnotation.class);
        else if (zipEntryName.equals("var_fa_ann.tsv"))
            dataSource.variantFunctionalAnalysisAnnotations = parseTSVUnivocity(stream,
                                                                                VariantFunctionalAnalysisAnnotation.class);
        else if (zipEntryName.equals("var_pheno_ann.tsv"))
            dataSource.variantPhenotypeAnnotations = parseTSVUnivocity(stream, VariantPhenotypeAnnotation.class);
    ...
    private <T> List<T> parseTSVUnivocity(final InputStream stream, final Class<T> clazz) {
