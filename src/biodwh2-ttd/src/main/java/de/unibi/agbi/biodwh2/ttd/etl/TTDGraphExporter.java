...
public class TTDGraphExporter extends GraphExporter<TTDDataSource> {
    ...
    public TTDGraphExporter(TTDDataSource dataSource) {
        super(dataSource);
    }
    ...
    protected boolean exportGraph(Workspace workspace, Graph graph) throws ExporterException {
        ...
        final Set<String> setOfPathwayIds = new HashSet<String>();
        final Map<String, Target> dictionaryTargetIdToTarget = new HashMap<String, Target>();
        ...
        try {
            extractDrugFlatFile(workspace, graph, dictionaryDrugIdToDrug);
        } catch (IOException e) {
            throw new ExporterException("Failed to export TTD Flat File Drug", e);
        }
        try {
            extractDrugFlatFileCrossRef(workspace, graph, dictionaryDrugIdToDrug);
        } catch (IOException e) {
            throw new ExporterException("Failed to export TTD Flat File Drug Cross", e);
        }
        try {
            extractDrugFlatFileSynonyms(workspace, graph, dictionaryDrugIdToDrug);
        } catch (IOException e) {
            throw new ExporterException("Failed to export TTD Flat File Drug synonyms", e);
        }
        try {
            extractDrugInfoFromDrugDiseaseFlatFile(workspace, graph, dictionaryDrugIdToDrug);
        } catch (IOException e) {
        ...
        try {
            extractTargetFlatFileUniprot(workspace, graph, dictionaryTargetIdToTarget);
        } catch (IOException e) {
            throw new ExporterException("Failed to export TTD Flat File Target uniprot", e);
        }
        try {
            extractTargetNameFromTargetDiseaseFlatFile(workspace, graph, dictionaryTargetIdToTarget);
        } catch (IOException e) {
            throw new ExporterException("Failed to export TTD Flat File Target disease", e);
        }

        addTargetToData(workspace, graph, dictionaryTargetIdToTarget);


        Set<String> setOfTargetIds = new HashSet<String>();
        setOfTargetIds.addAll(dictionaryTargetIdToTarget.keySet());
        ...
        addDrugToData(workspace, graph, dictionaryDrugIdToDrug);


        final Set<String> setOfDrugIds = new HashSet<String>();
        setOfDrugIds.addAll(dictionaryDrugIdToDrug.keySet());
        ...
        addTargetDrugEdges(workspace, graph, dictionaryTargetIdToDrugsToEdgeInfo, DRUG_LABEL);
        addTargetDrugEdges(workspace, graph, dictionaryTargetIdToCompoundToEdgeInfo, COMPOUND_LABEL);

        // pathway and target-pathway edges

        addTargetPathwayToData(workspace, graph, dictionaryTargetIdToPathways);
        ...
        try {
            extractDrugDiseaseFlatFile(workspace, graph, setOfDiseaseIds, setOfDrugIds);
        } catch (IOException e) {
            throw new ExporterException("Failed to export TTD Flat File drug disease", e);
        }
    ...
    private Set<String> exportBiomarkerDiseaseTSV(final Workspace workspace, final Graph graph) {
        final Set<String> setOfBiomarkerIds = new HashSet<String>();
        final Set<String> setOfDiseaseIds = new HashSet<String>();
    ...
    private boolean extractDrugDiseaseFlatFile(final Workspace workspace, final Graph graph,
                                               final Set<String> setOfDiseaseIds,
                                               final Set<String> setOfDrugIds) throws IOException {
        if (LOGGER.isInfoEnabled())
    ...
    private void exportPathwayKeggTargetTSV(final Workspace workspace, final Graph graph, String fileName,
        ...
        for (final KEGGPathwayToTarget entry : parseTsvFile(workspace, KEGGPathwayToTarget.class, fileName)) {
            ...
            if (!dictionaryTargetIdToPathways.containsKey(targetId))
                dictionaryTargetIdToPathways.put(targetId, new ArrayList<String>());
     ...
     */
    private void addTargetPathwayToData(final Workspace workspace, final Graph graph,
                                        final Map<String, List<String>> dictionaryTargetIdToPathways) {
    ...
    private void prepareEdgeInformationForDrugCompoundToTarget(final Graph graph,
        ...
        if (dictionaryTargetIdToNodeToEdgeInfo.containsKey(targetId) && dictionaryTargetIdToNodeToEdgeInfo.get(targetId)
            ...
            dictionaryTargetIdToNodeToEdgeInfo.put(targetId, new HashMap<String, TargetDrugEdge>());
            dictionaryTargetIdToNodeToEdgeInfo.get(targetId).put(nodeId, targetDrugEdge);
        }
     ...
     */
    private void addTargetDrugEdges(final Workspace workspace, final Graph graph,
                                    final Map<String, Map<String, TargetDrugEdge>> dictionaryTargetIdToDrugsToEdgeInfo,
    ...
    private String getTsvContentFromSheet(final Sheet sheet) throws ParserFormatException, IOException {
        ...
        for (final Row row : sheet.read()) {
            ...
            if (!foundHeaderCell) {
                ...
                if ("TargetID".equals(firstCellValue))
        ...
        try {
            for (final DrugTargetMapping entry : tryLoadXlsxTable(workspace, TTDUpdater.TARGET_COMPOUND_MAPPIN_XLSX,
                ...
                if (dictionaryTargetIdToDrugsToEdgeInfo.containsKey(targetId) &&
                    ...
                    dictionaryTargetIdToDrugsToEdgeInfo.put(targetId, new HashMap<String, TargetDrugEdge>());
                    dictionaryTargetIdToDrugsToEdgeInfo.get(targetId).put(drugId, targetDrugEdge);
                }
     ...
     */
    private void addTargetToData(final Workspace workspace, final Graph graph,
     ...
     */
    private void extractPathwayInformation(final Workspace workspace, final Graph graph, List<String> listOfPathways,
    ...
    private boolean extractTargetFlatFile(final Workspace workspace, final Graph graph,
        ...
        for (final FlatFileTTDEntry entry : reader) {
            ...
            if (entry.properties.get("KEGGPATH") != null) {
                extractPathwayInformation(workspace, graph, entry.properties.get("KEGGPATH"), setOfPathwayIds, "KEGG",
                                          targetId, dictionaryTargetIdToPathways);
            }
            if (entry.properties.get("WIKIPATH") != null) {
                extractPathwayInformation(workspace, graph, entry.properties.get("WIKIPATH"), setOfPathwayIds,
                                          "WikiPathway", targetId, dictionaryTargetIdToPathways);
            }
            if (entry.properties.get("WHIZPATH") != null) {
                extractPathwayInformation(workspace, graph, entry.properties.get("WHIZPATH"), setOfPathwayIds,
                                          "PathWhiz Pathway", targetId, dictionaryTargetIdToPathways);
            }
            if (entry.properties.get("REACPATH") != null) {
                extractPathwayInformation(workspace, graph, entry.properties.get("REACPATH"), setOfPathwayIds,
                                          "Reactome", targetId, dictionaryTargetIdToPathways);
            }
            if (entry.properties.get("NET_PATH") != null) {
                extractPathwayInformation(workspace, graph, entry.properties.get("NET_PATH"), setOfPathwayIds,
                                          "NetPathway", targetId, dictionaryTargetIdToPathways);
            }
            if (entry.properties.get("INTEPATH") != null) {
                extractPathwayInformation(workspace, graph, entry.properties.get("INTEPATH"), setOfPathwayIds,
                                          "Pathway Interact", targetId, dictionaryTargetIdToPathways);
            }
            if (entry.properties.get("PANTPATH") != null) {
                extractPathwayInformation(workspace, graph, entry.properties.get("PANTPATH"), setOfPathwayIds,
                                          "PANTHER Pathway", targetId, dictionaryTargetIdToPathways);
            }
            if (entry.properties.get("BIOCPATH") != null) {
                extractPathwayInformation(workspace, graph, entry.properties.get("BIOCPATH"), setOfPathwayIds, "BioCyc",
                                          targetId, dictionaryTargetIdToPathways);
            }
     ...
     */
    private boolean extractTargetNameFromTargetDiseaseFlatFile(final Workspace workspace, final Graph graph,
     ...
     */
    private boolean extractTargetFlatFileUniprot(final Workspace workspace, final Graph graph,
                                                 final Map<String, Target> dictionaryTargetIdToTarget) throws IOException {
        if (LOGGER.isInfoEnabled())
     ...
     */
    private void addDrugToData(final Workspace workspace, final Graph graph,
                               final Map<String, Drug> dictionaryDrugIdToDrug) {
     ...
     */
    private boolean extractDrugFlatFile(final Workspace workspace, final Graph graph,
                                        final Map<String, Drug> dictionaryDrugIdToDrug) throws IOException {
        if (LOGGER.isInfoEnabled())
     ...
     */
    private boolean extractDrugInfoFromDrugDiseaseFlatFile(final Workspace workspace, final Graph graph,
                                                           final Map<String, Drug> dictionaryDrugIdToDrug) throws IOException {
        if (LOGGER.isInfoEnabled())
            LOGGER.info("Export Drug-Disease Flat File ...");
        final FlatFileWithoutIDTTDReader reader = new FlatFileWithoutIDTTDReader(
     ...
     */
    private boolean extractDrugFlatFileCrossRef(final Workspace workspace, final Graph graph,
                                                final Map<String, Drug> dictionaryDrugIdToDrug) throws IOException {
        if (LOGGER.isInfoEnabled())
     ...
     */
    private boolean extractDrugFlatFileSynonyms(final Workspace workspace, final Graph graph,
                                                final Map<String, Drug> dictionaryDrugIdToDrug) throws IOException {
        if (LOGGER.isInfoEnabled())
